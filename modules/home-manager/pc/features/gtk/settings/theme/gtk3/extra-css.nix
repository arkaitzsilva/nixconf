{
  flake.modules.homeManager.pc = { lib, config, ... }: lib.mkIf config.gtk.enable {
    gtk.gtk3.extraCss = with config.scheme.withHashtag; ''
      @define-color window_bg_color ${base01};
      @define-color window_fg_color ${base05};
      @define-color view_bg_color ${base00};
      @define-color view_fg_color ${base06};
      @define-color accent_bg_color ${base0C};
      @define-color accent_fg_color ${base06};
      @define-color headerbar_bg_color ${base02};
      @define-color headerbar_fg_color ${base06};
      @define-color headerbar_backdrop_color @window_bg_color;
      @define-color popover_bg_color ${base03};
      @define-color popover_fg_color @view_fg_color;
      @define-color dialog_bg_color ${base03};
      @define-color dialog_fg_color @view_fg_color;
      @define-color card_bg_color ${base03};
      @define-color card_fg_color @view_fg_color;
      @define-color sidebar_bg_color ${base02};
      @define-color sidebar_fg_color ${base06};
      ...
      decoration {
        box-shadow: 0 3px 12px 1px rgba(0, 0, 0, 0.7), 0 0 0 1px mix(${base06},@window_bg_color,0.9);
      }
      decoration:backdrop {
        box-shadow: 0 3px 12px 1px transparent, 0 2px 6px 2px rgba(0, 0, 0, 0.4), 0 0 0 1px mix(${base06},@window_bg_color,0.95);
      }
      .tiled decoration, .tiled-top decoration, .tiled-bottom decoration, .tiled-right decoration, .tiled-left decoration {
        box-shadow: 0 0 0 1px mix(${base06},@window_bg_color,0.95), 0 0 0 20px transparent;
      }
      messagedialog.csd decoration, .csd.popup decoration, .maximized .csd.popup decoration {
        box-shadow: 0 1px 2px rgba(0, 0, 0, 0.8), 0 0 0 1px alpha(mix(${base06},@window_bg_color,0.9),0.9);
      }
      ...
      .ssd decoration {
        box-shadow: 0 0 0 1px mix(${base06},@window_bg_color,0.9);
      }
      .ssd decoration:backdrop {
        box-shadow: 0 0 0 1px mix(${base06},@window_bg_color,0.95);
      }
